...
For a checkpoint that passes its qualification gate, `TCNAdvisor` returns
`status=RESEARCH_ONLY`, `execution_eligible=false`, `can_authorize_order=false`.
For a checkpoint that fails the gate it returns `status=UNAVAILABLE`, also with
both flags false. **A refusal is the expected outcome for most checkpoints**,
including every slow-line checkpoint as the code stands today (see section 9).
...
- If a GPU is available on this machine, install the CUDA build of torch. This
  is a **template, not a runnable command** -- pick the CUDA tag that matches
  the installed driver (`nvidia-smi` reports the driver and the maximum CUDA
  version it supports), then substitute it:
  ```powershell
  & ".\.venv\Scripts\python.exe" -m pip install torch --index-url https://download.pytorch.org/whl/cu<TAG>
  # example tag shape: cu126, cu128. Confirm the current list at
  # https://pytorch.org/get-started/locally/ before running.
  ```
  **Verify `torch.cuda.is_available()` before and after**; do not trust the
  install message.
...
## 4. The missing dataset chain

The reproduction block in the research report references five files. Everything
under `backend/reports/` is git-ignored, so they were lost with the working
copy. State verified on 2026-09-18:

| Path | State |
| --- | --- |
| `backend/reports/mb_tcn_dataset.csv` | **present** -- regenerated by the validation pass, 129,531 rows |
| `backend/reports/mb_barrier_targets.npz` | **present** -- regenerated by the validation pass, 37,151 / 9,820 valid labels |
| `backend/reports/binance_full_dataset.csv` | **absent, no producer** |
| `backend/reports/binance_barrier_targets.npz` | **absent** (needs the file above) |
| `backend/reports/tcn_barrier_final/local_best.pt` | **absent** (needs a train) |

The two present files are git-ignored so they are local only: do not assume they
survive to another machine. Their producing commands are listed below and both
run clean. The remaining three are the real blockers, and the next section
explains why.
